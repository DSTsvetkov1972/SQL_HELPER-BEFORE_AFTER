...
def get_svod_table_columns(svod_table, container_field=None, order_field=None):
    ...
    for n, info in enumerate(svod_table_info, 1):
        ...
        if column_name == container_field.replace('`', ''):
            ...
            column_name_processed = f"\n\treplace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(upperUTF8(`{ { column_name } }`),' ',''),'	',''), ' ', ''), '　', ''),'Т','T'),'К','K'),'О','O'),'Е','E'),'Р','P'),'С','C'), 'Н', 'H'), 'В', 'B'), 'Х', 'X') AS `{ column_name }`{str_end}"
